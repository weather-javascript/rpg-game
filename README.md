...
管理者パネルを使うには Firebase カスタムクレームの設定が必要です。
以下の手順で設定してください。

### ステップ1: Cloud Functions をデプロイ
...
**Firebase Console** → Firestore → `admins` コレクションに
管理者にしたいユーザーの **UID** でドキュメントを作成します。

```
コレクション: admins
ドキュメントID: <管理者のUID>   ← Firebase AuthのUIDをコピー
フィールド: role = "admin"     （値は何でもOK、ドキュメントの存在だけで機能する）
```

UIDは Firebase Console → Authentication → ユーザー一覧で確認できます。

### ステップ3: 管理者ユーザーが再ログイン

ゲームをリロードするとログイン時に `refreshAdminClaim` が自動で呼ばれ、
カスタムクレーム（`admin: true`）がトークンに反映されます。

> **Cloud Functions が未デプロイの場合のフォールバック:**
> Firestoreルールの `isAdmin()` は `admins/{uid}` ドキュメントの存在でも
> 管理者と判定するため、Functions なしでも動作します。
> ただしトークンベースでないためルールの読み取りが1回増えます。
