...
function spawnEnemies(dungeon: DungeonMaster, areaIdx: number, _kxPhase?: number, volcanoRoute?: string): EnemyState[] {
  ...
  const maxEnemies = dungeon.id === 'sky_castle_ex' ? 15 : dungeon.id === 'volcano' ? 6 : dungeon.id === 'astral_nox' ? 8 : dungeon.id === 'chaite' ? 9 : 3;
...
function getEquipmentHint(dungeon: DungeonMaster): string | null {
  ...
  if (dungeon.tier === 'cosmic') return '🚀 ルートごとに装備を切り替えること。対策なしでは突破不可能';
  return null;
}
...
function DungeonCard({ dungeon, selected, onSelect, playerLevel, clearedCount, isUnlocked, isRecommended }: {
  ...
  const tierColors: Record<string, string> = {
    ...
    super: '#e05555', extreme: '#9b6df0', volcano: '#ff6b35', cosmic: '#00c8ff',
  };
...
export function DungeonScreen() {
  ...
  // ffEncounterProfileId は現在未使用（profileId は runState.dungeonId から参照可能）
