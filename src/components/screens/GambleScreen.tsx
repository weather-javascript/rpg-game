...
export function GambleScreen() {
  ...
  const vocationGambleMult = player ? getPlayerPowerProfile(player).gambleMult : 1; // ver3.0.0: 賭博師職業/特性/ペットの配当ボーナス
  ...
  return (
    <div className="gamble-shell" style={{ padding: isMobile ? '10px 8px 18px' : '12px 8px' }}>
      ...
      <div className="gamble-card gamble-panel" style={{ background: 'linear-gradient(180deg, rgba(28,34,53,0.96), rgba(22,27,38,0.96))', border: '1px solid #2d3752', borderRadius: 10, padding: 14 }}>
        ...
        {activeGame === 'chohan'       && <ChohanPanel    bet={bet} onResult={handleResult} onJackpotContrib={handleJackpotContrib} multiplierBonus={(gambleMultipliers['chohan'] ?? 1.0) * (ticketActive ? 2 : 1) * (dailyCasino.id === 'chohan' ? dailyCasino.bonus : 1) * (activeEvent?.gameId === 'chohan' ? activeEvent.bonusValue : 1) * vocationGambleMult} onLockChange={setChohanBetLocked} />}
        {activeGame === 'chinchiro'    && <ChinchiroPanel bet={bet} onResult={handleResult} onJackpotContrib={handleJackpotContrib} multiplierBonus={(gambleMultipliers['chinchiro'] ?? 1.0) * (ticketActive ? 2 : 1) * (dailyCasino.id === 'chinchiro' ? dailyCasino.bonus : 1) * (activeEvent?.gameId === 'chinchiro' ? activeEvent.bonusValue : 1) * vocationGambleMult} onLockChange={setChinchiroBetLocked} />}
        {activeGame === 'coin_flip'    && <CoinFlipPanel  bet={bet} onResult={handleResult} onJackpotContrib={handleJackpotContrib} multiplierBonus={(gambleMultipliers['coin_flip'] ?? 1.0) * (ticketActive ? 2 : 1) * (dailyCasino.id === 'coin_flip' ? dailyCasino.bonus : 1) * vocationGambleMult} onLockChange={setCoinFlipBetLocked} />}
        {activeGame === 'slot'         && <SlotPanel onResult={handleResult} onJackpotContrib={handleJackpotContrib} multiplierBonus={(gambleMultipliers['slot_machine'] ?? 1.0) * (ticketActive ? 2 : 1) * (dailyCasino.id === 'slot' ? dailyCasino.bonus : 1) * (activeEvent?.gameId === 'slot' ? activeEvent.bonusValue : 1) * vocationGambleMult} onLockChange={setSlotBetLocked} />}
        {activeGame === 'treasure_box' && <TreasureBoxPanel onResult={handleResult} onJackpotContrib={handleJackpotContrib} multiplierBonus={(gambleMultipliers['treasure_box'] ?? 1.0) * (ticketActive ? 2 : 1) * vocationGambleMult} onLockChange={setTreasureBoxBetLocked} />}
        {activeGame === 'poker'        && <PokerPanel    bet={bet} onResult={handleResult} onJackpotContrib={handleJackpotContrib} multiplierBonus={(gambleMultipliers['poker'] ?? 1.0) * (ticketActive ? 2 : 1) * (dailyCasino.id === 'poker' ? dailyCasino.bonus : 1) * (activeEvent?.gameId === 'poker' ? activeEvent.bonusValue : 1) * vocationGambleMult} onBetLock={setPokerBetLocked} />}
        ...
        {activeGame === 'mines'        && <MinesPanel     bet={bet} onResult={handleResult} onJackpotContrib={handleJackpotContrib} multiplierBonus={(gambleMultipliers['mines'] ?? 1.0) * (ticketActive ? 2 : 1) * (dailyCasino.id === 'mines' ? dailyCasino.bonus : 1) * (activeEvent?.gameId === 'mines' ? activeEvent.bonusValue : 1) * vocationGambleMult} onLockChange={setMinesBetLocked} />}
        {activeGame === 'dice_race'    && <DiceRacePanel  bet={bet} onResult={handleResult} onJackpotContrib={handleJackpotContrib} multiplierBonus={(gambleMultipliers['dice_race'] ?? 1.0) * (ticketActive ? 2 : 1) * (dailyCasino.id === 'dice_race' ? dailyCasino.bonus : 1) * (activeEvent?.gameId === 'dice_race' ? activeEvent.bonusValue : 1) * vocationGambleMult} onLockChange={setDiceRaceBetLocked} />}
        {activeGame === 'roulette'     && <RoulettePanel  bet={bet} onResult={handleResult} onJackpotContrib={handleJackpotContrib} multiplierBonus={(gambleMultipliers['roulette'] ?? 1.0) * (ticketActive ? 2 : 1) * (dailyCasino.id === 'roulette' ? dailyCasino.bonus : 1) * (activeEvent?.gameId === 'roulette' ? activeEvent.bonusValue : 1) * vocationGambleMult} onLockChange={setRouletteBetLocked} />}
        {activeGame === 'blackjack'    && <BlackjackPanel bet={bet} onResult={handleResult} onJackpotContrib={handleJackpotContrib} multiplierBonus={(gambleMultipliers['blackjack'] ?? 1.0) * (ticketActive ? 2 : 1) * (dailyCasino.id === 'blackjack' ? dailyCasino.bonus : 1) * (activeEvent?.gameId === 'blackjack' ? activeEvent.bonusValue : 1) * vocationGambleMult} onLockChange={setBlackjackBetLocked} />}
        {activeGame === 'scratch'      && <ScratchPanel   bet={bet} onResult={handleResult} onJackpotContrib={handleJackpotContrib} multiplierBonus={(gambleMultipliers['scratch'] ?? 1.0) * (ticketActive ? 2 : 1) * (dailyCasino.id === 'scratch' ? dailyCasino.bonus : 1) * (activeEvent?.gameId === 'scratch' ? activeEvent.bonusValue : 1) * vocationGambleMult} onLockChange={setScratchBetLocked} />}
        {activeGame === 'race'         && <RacePanel      bet={bet} onResult={handleResult} onJackpotContrib={handleJackpotContrib} multiplierBonus={(gambleMultipliers['race'] ?? 1.0) * (ticketActive ? 2 : 1) * (dailyCasino.id === 'race' ? dailyCasino.bonus : 1) * (activeEvent?.gameId === 'race' ? activeEvent.bonusValue : 1) * vocationGambleMult} onLockChange={setRaceBetLocked} />}
        {activeGame === 'pvp'          && (
          <>
