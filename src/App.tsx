...
function StatusBar() {
  ...
  return (
    <header style={{display:'flex',alignItems:'center',gap:10,background:'#161b26',borderBottom:'1px solid #2d3752',padding:'6px 12px',position:'sticky',top:0,zIndex:100}}>
      ...
      <div style={{display:'flex',gap:6,flex:1}}>
        <div style={{display:'flex',alignItems:'center',gap:3,flex:1}} title={`HP: ${player.stats.hp}/${player.stats.maxHp}`}>
          <span style={{fontSize:'0.7rem'}}>❤️</span>{bar(hpPct,'#e05555')}
        </div>
        <div style={{display:'flex',alignItems:'center',gap:3,flex:1}} title={`満腹度: ${player.stats.satiety}/${player.stats.maxSatiety}`}>
          <span style={{fontSize:'0.7rem'}}>🍖</span>{bar(satPct, satPct < 20 ? '#e05555' : '#f0a830')}
        </div>
      ...
      {TABS.map(tab => (
        <button
          ...
          style={{
            ...
            cursor:'pointer',fontSize:'0.65rem',transition:'all 0.2s',
          }}
        >
