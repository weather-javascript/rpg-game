...
export const GATHER_NODE_MASTER: Record<string, GatherNodeMaster> = {
  ...
  stone_deposit:  { id:'stone_deposit',  name:'岩場',     description:'地面にある岩の塊。',       icon:'🪨', requiredSkill:{skillId:'mining',     minLevel:1},  cooldownMs:4000,  staminaCost:6,  drops:[{itemId:'stone',     baseRate:1.0, minAmount:1, maxAmount:3},{itemId:'coal',  baseRate:0.15, minAmount:1, maxAmount:2}] },
  iron_vein:      { id:'iron_vein',      name:'鉄鉱脈',   description:'鉄を含む鉱脈。',           icon:'⛏️', requiredSkill:{skillId:'mining',     minLevel:10}, cooldownMs:8000,  staminaCost:12, drops:[{itemId:'iron_ore',  baseRate:1.0, minAmount:1, maxAmount:3, skillRateBonus:0.1},{itemId:'gold_ore', baseRate:0.05, minAmount:1, maxAmount:1}] },
  gold_vein:      { id:'gold_vein',      name:'金鉱脈',   description:'希少な金を含む鉱脈。',     icon:'✨', requiredSkill:{skillId:'mining',     minLevel:25}, cooldownMs:15000, staminaCost:20, drops:[{itemId:'gold_ore',  baseRate:1.0, minAmount:1, maxAmount:3, skillRateBonus:0.12}] },
};
...
export const DUNGEON_MASTER: Record<string, DungeonMaster> = {
  ...
  underground_fortress: {
    id:'underground_fortress', name:'地下要塞', description:'中級の隠しダンジョン。防御貫通攻撃が初登場。上級攻略後の実力が必要。',
  ...
  garden: {
    id:'garden', name:'箱庭庭園', description:'モグラのドリュウが死因の大半を占める。開幕無敵に注意。',
  ...
  frozen_cave: {
    id:'frozen_cave', name:'冷焦洞穴', description:'属性攻撃システムあり。炎系→極冷、氷系→極焦が有効。裏ボス「零」は洞窟そのもの。',
  ...
  sky_castle: {
    id:'sky_castle', name:'天空城', description:'超上級ダンジョン。敵の数は少ないが一体ずつが強い。遠距離攻撃推奨。',
  ...
  dragons_lair: {
    id:'dragons_lair', name:'ドラゴンの塒', description:'伝説のダンジョン。最強の冒険者のみが踏み込める。',
...
export const GAMBLE_MASTER: Record<string, GambleMaster> = {
  chohan: {
    ...
    icon:'🎲', type:'chohan', minBet:10, maxBet:50000, returnRate:0.95,
    rewardTable:[
      { label:'丁（偶数）当たり！', probability:0.4750, multiplier:2.0, symbols:['⚀⚂'] },
      { label:'半（奇数）当たり！', probability:0.4750, multiplier:2.0, symbols:['⚁⚃'] },
      { label:'ゾロ目（引き分け）', probability:0.0500, multiplier:0.5, symbols:['⚀⚀'] },
    ],
  },
  chinchiro: {
    id:'chinchiro', name:'チンチロリン', description:'ダイス3個で役を作る。役が高いほど配当アップ。',
    icon:'🎯', type:'chinchiro', minBet:50, maxBet:100000, returnRate:0.93,
    rewardTable:[
      { label:'ピンゾロ（1-1-1）',    probability:0.004,  multiplier:10.0, symbols:['1','1','1'] },
      { label:'シゴロ（4-5-6）',       probability:0.027,  multiplier:5.0,  symbols:['4','5','6'] },
      { label:'ゾロ目（2-6）',         probability:0.083,  multiplier:3.0,  symbols:['🎲','🎲','🎲'] },
      { label:'ヒフミ（1-2-3）',       probability:0.027,  multiplier:0.0,  symbols:['1','2','3'] },
      { label:'目なし（ゾロ目以外）',  probability:0.200,  multiplier:0.0,  symbols:['💨','💨','💨'] },
      { label:'通常（数字目）',        probability:0.659,  multiplier:1.5,  symbols:['🎲','🎲','🔢'] },
    ],
  },
  jackpot: {
    id:'jackpot', name:'ジャックポット', description:'賭け金の一部がプールに追加。超低確率で大当たり！',
    icon:'🌟', type:'jackpot', minBet:100, maxBet:100, returnRate:0.50,
    rewardTable:[
      { label:'🌟 JACKPOT!! 🌟', probability:0.001,  multiplier:0,    symbols:['🌟','🌟','🌟'] },
      { label:'はずれ',           probability:0.999,  multiplier:0,    symbols:['💨','💨','💨'] },
    ],
  },
  poker: {
    id:'poker', name:'簡易ポーカー', description:'5枚の手札を1回チェンジ。役を作って配当ゲット。',
    icon:'🃏', type:'poker', minBet:50, maxBet:50000, returnRate:0.97,
    rewardTable:[
      ...
      { label:'ワンペア（JJ以上）',           probability:0.21200, multiplier:1,   symbols:['🃏'] },
      { label:'ブタ（役なし）',               probability:0.00041, multiplier:0,   symbols:['🃏'] },
    ],
  },
  slot_machine: {
    id:'slot_machine', name:'スロットマシン', description:'3つのシンボルが揃えばジャックポット！',
    icon:'🎰', type:'slot', minBet:10, maxBet:10000, returnRate:0.92,
    rewardTable:[
      { label:'💰💰💰 JACKPOT',   probability:0.005, multiplier:50,  symbols:['💰','💰','💰'] },
      { label:'⭐⭐⭐ BIG WIN',   probability:0.020, multiplier:10,  symbols:['⭐','⭐','⭐'] },
      { label:'🍒🍒🍒 MIDDLE',   probability:0.050, multiplier:5,   symbols:['🍒','🍒','🍒'] },
      { label:'🍋🍋🍋 SMALL',    probability:0.100, multiplier:2,   symbols:['🍋','🍋','🍋'] },
      { label:'🍒🍒 2枚揃い',    probability:0.200, multiplier:1.2, symbols:['🍒','🍒','🔔'] },
      { label:'ハズレ',           probability:0.625, multiplier:0,   symbols:['💨','💨','💨'] },
    ],
  },
  treasure_box: {
    id:'treasure_box', name:'宝箱くじ', description:'宝箱を開けてみよう。何が出るかはお楽しみ。',
    icon:'📦', type:'treasure_box', minBet:100, maxBet:100, returnRate:0.85,
    rewardTable:[
      ...
      { label:'ゴールド大量',              probability:0.10, multiplier:5,   symbols:['💰'] },
      { label:'回復薬セット',              probability:0.30, multiplier:0,   itemRewards:[{itemId:'health_potion', amount:5}],          symbols:['🧪'] },
      { label:'少しゴールド',              probability:0.30, multiplier:1.5, symbols:['🪙'] },
      { label:'ハズレ（空の宝箱）',        probability:0.24, multiplier:0,   symbols:['📭'] },
    ],
  },
  coin_flip: {
    id:'coin_flip', name:'コインフリップ', description:'表か裏か。シンプルな50/50ギャンブル。',
    icon:'🪙', type:'coin_flip', minBet:10, maxBet:50000, returnRate:0.96,
    rewardTable:[
      { label:'表（当たり）', probability:0.48, multiplier:2, symbols:['✨'] },
      { label:'裏（ハズレ）', probability:0.52, multiplier:0, symbols:['💨'] },
    ],
  },
};
